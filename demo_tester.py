...
user_ids = None
create_more_users = raw_input("Would you like to create new users ([y]|n)? ")
if not create_more_users.startswith("n"):
    num_of_users = int(raw_input("How many users would you like to create? "))
    print "Creating users..."
    user_ids = User.objects.create_test_users(num_of_users)
...
create_more_records = raw_input("Would you like to create new records ([y]|n)? ")
if not create_more_records.startswith("n"):
    ...
    Record.objects.create_random_records(records_mean, records_sigma, user_ids)
...
timeit(action.users_with_completes_denorm_raw)()
timeit(action.users_with_completes_denorm_best)()
