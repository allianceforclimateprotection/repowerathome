...
env.aws_key = local("echo $RAH_AWS_ACCESS_KEY", capture=True)
env.aws_secret = local("echo $RAH_AWS_SECRET_KEY", capture=True)
...
def _wait_for_resources(resources, check_increment=5, up_state="running", test_ssh=True):
    ...
    if test_ssh:
        with settings(hide("running", "warnings", "stderr"), warn_only=True):
            while any([local("ssh -o StrictHostKeyChecking=no ubuntu@%s 'echo TEST'" % r.public_dns_name).failed for r in resources], capture=True):
                time.sleep(check_increment)
                print ", ".join(["%s is %s" % (r, r.state) for r in resources])
