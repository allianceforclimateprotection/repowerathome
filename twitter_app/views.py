...
from rah.views import forbidden
...
def main(request):
    if request.user.get_profile().twitter_access_token:
...
#     if not access_token:
#         return rah.views.forbidden(request, 'You need an access token to post a message to twitter.')
#     token = oauth.OAuthToken.from_string(access_token)
#     
#     auth = is_authenticated(CONSUMER, CONNECTION, token)
#     if auth:
#         creds = simplejson.loads(auth)
#         name = creds.get('name', creds['screen_name'])
        

def friend_list(request):
    users = []
    
    access_token = request.user.get_profile().twitter_access_token
    if not access_token:
        return forbidden(request, 'You need an access token to post a message to twitter.')
    token = oauth.OAuthToken.from_string(access_token)   
